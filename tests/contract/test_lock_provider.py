...
@pytest.mark.asyncio
async def test_memory_lock_acquire_release_and_owner_validation() -> None:
    clock = Clock()
    locks = MemoryLockProvider(clock=lambda: clock.now)
...
@pytest.mark.asyncio
async def test_memory_lock_extend_requires_current_owner() -> None:
    clock = Clock()
    locks = MemoryLockProvider(clock=lambda: clock.now)
...
@pytest.mark.asyncio
async def test_memory_lock_expired_lock_can_be_reacquired_with_new_fencing_token() -> None:
    clock = Clock()
    locks = MemoryLockProvider(clock=lambda: clock.now)
...
@pytest.mark.asyncio
async def test_memory_lock_require_acquire_raises_stable_code_when_locked() -> None:
    ...
    with pytest.raises(AppError) as locked:
        await locks.require_acquire("scheduler:hourly", owner_token="owner-2", ttl_seconds=60)
...
@pytest.mark.asyncio
async def test_memory_lock_rejects_invalid_ttl_and_key() -> None:
    locks = MemoryLockProvider()

...
- Status: `connected`
- Done: settings、profile 校验、secret provider、HTTP client credential secret ref、脱敏诊断、启动期安全检查、local/private/cloud profile 模板输出、task queue/scheduler/tenant lifecycle 运行参数、profile security hardening 清单、配置 drift-check、运行时漂移告警输出、profile 派生部署产物渲染、部署产物写入命令和 release checkpoint drift gate 已落地。
- Next: _none_
...
```env
DATABASE__URL=postgres://user:pass@localhost:5432/fastapi_foundation
DATABASE__READ_URL=postgres://readonly:pass@localhost:5432/fastapi_foundation
...
DEPENDENCIES__RABBITMQ_URL=amqp://rabbitmq:rabbitmq@rabbitmq:5672/%2F
...
`DEPENDENCIES__REDIS_URL` 启用 Redis runtime 装配：app 启动时创建 `redis.asyncio` client，挂载 Redis cache/lock provider，并在 `/readyz` 中执行 Redis `PING` 探活。未配置时不会强制连接 Redis。
`DEPENDENCIES__RABBITMQ_URL` 启用 RabbitMQ MQ client 装配：app 启动时挂载 `app.state.rabbitmq_client`，并在 `/readyz` 中执行 RabbitMQ 连接/通道探活。未配置时不会强制连接 RabbitMQ。
...
`check-config` 会接受生产 profile 的外部 secret reference，但启动期 `create_app()` 仍必须通过 secret provider 解析到真实密钥后才能通过 `validate_startup_settings()`。cloud profile 的 `check-config` 还会输出 `details.database_tenant_guard`，并在缺少 PostgreSQL 或 `session_variable` fallback 时失败。
...
模板值中的 `${PLACEHOLDER}` 会作为非空通配符处理，因此 `DATABASE__URL=postgresql+asyncpg://app:${DATABASE_PASSWORD}@postgres:5432/fastapi_foundation` 可匹配运行时注入的真实密码。报告中的数据库 URL password、secret、token、password 字段必须脱敏。
...
`core config write-artifacts --profile <profile> --target <docker-compose|helm-values|systemd> --output-dir <dir> --json` 会把同一套部署产物写入目标目录。Docker Compose 目标会写出 `Dockerfile` 和 `docker-compose.<profile>.yml`；Helm 目标会写出 `helm/fastapi-foundation/Chart.yaml`、`values.<profile>.yaml` 和 `templates/workload.yaml`。写入时会约束 artifact 路径必须落在 `--output-dir` 下。

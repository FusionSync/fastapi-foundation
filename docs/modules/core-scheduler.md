...
- `ScheduleStateRepository.plan_cron_due_slots()` 支持 `skip`、`run_once` 和 `catch_up_limited` misfire policy；`catch_up_limited` 可通过 `trigger_config.misfire_limit` 限制单轮补偿数量。

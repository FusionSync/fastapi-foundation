...
- `FileService.download_bytes()` 执行 tenant lifecycle `file_download` gate，再通过 `FileResourceAuthorizationAdapter` 校验 tenant/resource 实例，并要求 `file.download` 权限后读取 storage。

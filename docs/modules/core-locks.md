...
- Status: `partial`
- Done: lock provider 抽象、内存实现、数据库表实现、scheduler trigger 并发保护，以及 outbox/migration 跨进程锁使用点已落地。
- Next:
  - [ ] 接 Redis 或 PostgreSQL advisory lock provider。
  - [ ] 为 audit hash chain 定义跨进程锁使用点。
...
- `OutboxDispatcher` 可注入 `LockProvider`，在批量领取前获取 `outbox:dispatch` 粗粒度锁；`apply_migrations()` 在执行真实 migration 前通过注入的 provider 获取 `migrations:apply` 锁。

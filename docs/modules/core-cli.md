...
- Status: `partial`
- Done: `check-app`、`list-apps`、permissions、migrate plan/preflight/dry-run/apply/status/drift-check、显式 Alembic apply、outbox dispatch/dead-letter、outbox-dispatcher run、scheduler run-once、tasks、operations/smoke 等命令骨架已接入。
- Next:
  - [ ] 补 server/worker/migrate 角色启动命令和 scheduler 后台 loop。

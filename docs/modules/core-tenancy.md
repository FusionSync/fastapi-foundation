...
`resolve_current_tenant()` 是唯一入口，顺序和门禁如下：

```text
1. 解析认证主体，得到 current_user。
2. 读取 token organization claim、X-Tenant-Id、用户默认租户。
3. 如果 header 与 token organization claim 冲突，返回 403 + TENANT_CONTEXT_CONFLICT。
4. 如果用户不是该租户 active member，返回 403 + TENANT_ACCESS_DENIED。
5. 如果租户状态不是当前操作允许状态，返回 403 + TENANT_STATE_FORBIDDEN。
6. 写入冻结后的 RequestContext.tenant_id。
```

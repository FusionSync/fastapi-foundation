...
当前 `core worker` 命令提供配置级健康检查，不强制依赖数据库连接；`core worker --run --max-iterations <n>`、`core scheduler --run-once`、`core scheduler --run --max-iterations <n>` 和 `core outbox-dispatcher --run` 可在 local/CI 中执行有限轮运行验证。`worker --run`、`scheduler --run` 和 `outbox-dispatcher --run` 传入 `--instance-id` 时会写入 `process_heartbeats`。
